    ...
    */
      ...
      )
    {
      ...
      double fontSize = 0;
      {
        ...
        //@ACLS: Document.Form.Resources.Fonts[fontName] is sometimes null
        var fonts = Document.Form.Resources.Fonts;
        var font = fonts[fontName];
        if (font == null) {
            // find font by name
            foreach (var kvPair in fonts) {
                if (kvPair.Value.Name == fontName.StringValue) {
                    font = kvPair.Value;
                    break;
                }
            }
        ...
        if (font == null) {
            // font not found, so find by partial name
            foreach (var kvPair in fonts) {
                if (kvPair.Value.Name.StartsWith(fontName.StringValue)) {
                    font = kvPair.Value;
                    break;
                }
            }
        ...
        if (font == null) {
            // font still not found, so use default font
            font = fonts[new PdfName("default")];
        }

        normalAppearance.Resources.Fonts[fontName] = font;
      }
