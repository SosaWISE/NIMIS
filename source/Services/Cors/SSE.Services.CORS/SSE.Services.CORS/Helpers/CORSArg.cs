...
namespace SSE.Services.CORS.Helpers
{
	public class CORSArg
	{
		...
		public static bool ArgumentValidation<T>(List<CORSArg> argArray, out SosCORSResult<T> result)
		{
			/** Initialize. */
			var sb = new StringBuilder();
			var passesValidation = true;
			result = new SosCORSResult<T>((int)SosResultCodes.ArgumentValidating
			...
			if (!passesValidation)
			{
				result = new SosCORSResult<T>((int)SosResultCodes.ArgumentValidationFailed
