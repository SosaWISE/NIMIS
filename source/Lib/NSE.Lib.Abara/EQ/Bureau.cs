...
using NSE.Lib.Abara.com.abarasoftware.blinkws.EQ;
using SOS.Data.SosCrm;
using SOS.Lib.Core.CreditReportService;
using SOS.Lib.Core.ErrorHandling;

namespace NSE.Lib.Abara.EQ
{
	public class Bureau
	{
		#region .ctor
		...
		/// </summary>
		/// <param name="oWSLead">IWSLead</param>
		/// <param name="oWSAddress">IWSAddress</param>
		/// <param name="season">IWSSeason</param>
		/// <param name="oRepInfo">IWSCreditReportInfo</param>
		/// <param name="szAdUsername">string</param>
		/// <param name="oMessageList">MessageController</param>
		...
		public InqueryResponse Execute(IWSLead oWSLead, IWSAddress oWSAddress, IWSSeason season, string szAdUsername, ref List<WSMessage> oMessageList, out IWSCreditReportInfo oRepInfo)
		{
			...
			var oEndPointAddress = new System.ServiceModel.EndpointAddress(Credentials.EndPointEqUri);
			var oService = new EquifaxWSSoapClient("EquifaxWSSoap", oEndPointAddress);
...
// ReSharper disable once PossibleNullReferenceException
				...
				SaveCr(oResult, oWSLead, oWSAddress, season, szAdUsername, ref oMessageList, out oRepInfo);

			}
			catch (Exception oEx)
			{
				oMessageList.Add(new WSMessage { DisplayMessage = string.Format("The following exception was thrown:{0}", oEx.Message), MessageType = ErrorMessageType.Exception, Ex = oEx, Title = "Critical Error" });
			}
		...
		private static void BindData(InqueryResponse oIqResponse, ReportResponse oResponse, ref List<WSMessage> oMessageList)
		{
			...
			oIqResponse.ReportOutcome = Main.GetReportOutcomeEQ(oResponse.ReportOutcome);
			oIqResponse.Result = Main.GetResultEQ(oResponse.Result);
			...
			if (!String.IsNullOrEmpty(oResponse.ErrorMessage))
			{
				...
				oMessageList.Add(new WSMessage{Title = "Abara Error", MessageType = ErrorMessageType.Critical, DisplayMessage = string.Format("Message:\r\n{0}", oResponse.ErrorMessage)});
			}
		...
		private static void SaveCr(InqueryResponse oIqResponse, IWSLead oWsLead, IWSAddress oWsAddress, IWSSeason season, string szAdUsername, ref List<WSMessage> oMessageList, out IWSCreditReportInfo oRepInfo)
		{
			try
			{
				...
				oRepInfo = new Main.WSCreditReportInfo
				{
					...
					BureauName = QL_CreditReportBureau.MetaData.EquafaxID,
