...
namespace SSE.Lib.SseGpsDeviceAPI.Test
{
	...
	public class CheckSumTest
	{
		...
		public void RawSentenceToSentenceTest()
		{
			...
			try
			{
				SseGpsDeviceAPI.Helper.SentenceParser.RawSentenceToSentence(String.Format("4343"));
				Assert.IsFalse(true, "Validation failed.  The sentence should not be less than 3 characters long.");
			}
			catch (SseGpsDeviceSentenceLengthException oEx)
			{
			...
			try
			{
				var temp = SseGpsDeviceAPI.Helper.SentenceParser.RawSentenceToSentence(String.Format("${0}*34343", SENTENCE));
			...
			try
			{
				SseGpsDeviceAPI.Helper.SentenceParser.RawSentenceToSentence(String.Format("{0}*34343", SENTENCE));
			}
			catch (SseGpsDeviceSentenceMissingDollarSign oEx)
			{
				Assert.IsTrue(true, oEx.Message);
			}
			...
			catch (SseGpsDeviceSentenceMissingChkSum oEx)
			{
				Assert.IsTrue(true, oEx.Message);
			}
		...
		public void GetCheckSumRawSentenceTestGood()
		{
			...
			string chkSum = "14";
			var chkSumVal = SseGpsDeviceAPI.Helper.SentenceParser.GetCheckSumRawSentence(string.Format("${0}*14", sentance));
			Assert.IsTrue(chkSumVal.Equals(chkSum), "'GetCheckSumRawSentence Test 001' failed checksum test.");

			/** Test 002. */
			sentance = "AVRSP,99999999,00";
			chkSum = "46";
			chkSumVal = SseGpsDeviceAPI.Helper.SentenceParser.GetCheckSumRawSentence(string.Format("${0}*46", sentance));
			...
			chkSum = "16";
			chkSumVal = SseGpsDeviceAPI.Helper.SentenceParser.GetCheckSumRawSentence(string.Format("${0}*16", sentance));
			Assert.IsTrue(chkSumVal.Equals(chkSum), "'GetCheckSumRawSentence Test 003' failed checksum test.");

			/** Test 004. */
			sentance = "AVREQ,00000000,0";
			chkSum = "61";
			chkSumVal = SseGpsDeviceAPI.Helper.SentenceParser.GetCheckSumRawSentence(string.Format("${0}*61", sentance));
			Assert.IsTrue(chkSumVal.Equals(chkSum), "'GetCheckSumrRawSentence Test 004' failed checksum test.");

			/** Test 005. */
			sentance = "AVALL,99999999,00,3";
			chkSum = "49";
			chkSumVal = SseGpsDeviceAPI.Helper.SentenceParser.GetCheckSumRawSentence(string.Format("${0}*49", sentance));
		...
		public void GetCheckSumRawSentenceTestBad()
		{
			...
			var chkSum = SseGpsDeviceAPI.Helper.SentenceParser.GetCheckSumRawSentence(string.Format("${0}*14", SENTANCE));
		...
		public void GetCheckSumTestGood()
		{
			...
			string sentance = "AVRMC,90007200,005142,r,4019.1472,N,11140.5435,W,0.00,0.00,151112,0,3774,27,1,0,0";
			string chkSum = "14";
			var chkSumVal = SseGpsDeviceAPI.Helper.SentenceParser.GetCheckSum(sentance);
			Assert.IsTrue(chkSumVal.Equals(chkSum), "'GetCheckSumRawSentence Test 001' failed checksum test.");

			/** Test 002. */
			sentance = "AVRSP,99999999,00";
			chkSum = "46";
			chkSumVal = SseGpsDeviceAPI.Helper.SentenceParser.GetCheckSum(sentance);
			Assert.IsTrue(chkSumVal.Equals(chkSum), "'GetCheckSumRawSentence Test 002' failed checksum test.");
