...
namespace SOS.Lib.AuthorizeNet.AIM.Requests
{
	...
	public abstract class GatewayRequest : IGatewayRequest
	{
		...
		#region IGatewayRequest Members
		...
		{
			Freight = string.Format("{0}<|>{1}<|>{2}", name, description, amount.ToString(CultureInfo.InvariantCulture));
			return this;
		}
		...
		{
			Duty = string.Format("{0}<|>{1}<|>{2}", name, description, amount.ToString(CultureInfo.InvariantCulture));
			return this;
		}
		...
		{
			Tax = string.Format("{0}<|>{1}<|>{2}", name, description, amount.ToString(CultureInfo.InvariantCulture));
			return this;
		}
		...
		/// </summary>
		/// <returns></returns>
		public IGatewayRequest AddLineItem(string itemID, string name, string description, int quantity, decimal price,
		                                   bool taxable)
		{
			String sFld = ApiFields.LineItem;
			string lineFormat = string.Format("{0}<|>{1}<|>{2}<|>{3}<|>{4}<|>{5}",
									itemID,
									name,
									description,
									quantity.ToString(CultureInfo.InvariantCulture),
									price.ToString(CultureInfo.InvariantCulture),
									taxable.ToString(CultureInfo.InvariantCulture));
