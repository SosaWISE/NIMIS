...
namespace NXS.DataServices.Sales
{
	public class BlahService
	{
		...
		public async Task<Result<List<SystemType>>> SystemTypesAsync(int officeid)
		{
			using (var db = DBase.Connect())
			{
				var sql = @"SELECT id, companyName, filename
					FROM systemTypes
					WHERE (officeId=0 OR officeId=@officeid)
					ORDER BY sequence, companyName";
				var items = (await db.QueryAsync<SystemType>(sql, new { officeid }).ConfigureAwait(false)).ToList();
				return new Result<List<SystemType>>(value: items);
			}
		}
		...
		public async Task<int> TrackLocationAsync(SalesTracking inputItem)//int salesRepId, int latitude, int longitude)
		{
			using (var db = DBase.Connect())
			{
				var sql = @"INSERT into salesTracking (salesRepId, latitude, longitude)
					VALUES(@salesRepId, @latitude, @longitude)";
				return await db.ExecuteAsync(sql, new { inputItem.salesRepId, inputItem.latitude, inputItem.longitude }).ConfigureAwait(false);
			}
		}
	}
}
