...
BEGIN
	...
	IF (NOT EXISTS (SELECT * FROM [WISE_AuthenticationControl].[dbo].AC_Sessions AS SESS WITH (NOLOCK) WHERE (SESS.SessionID = @SessionId) AND (SESS.SessionTerminated = 0) AND (SESS.LastAccessedOn BETWEEN @Start AND @End)))
	BEGIN
		...
		SELECT * FROM [WISE_AuthenticationControl].[dbo].[vwAC_UsersAppAuthentication] AS USR WITH (NOLOCK) WHERE (USR.UserID IS NULL);
	...
	SELECT 
		...
		, [WISE_HumanResource].dbo.fxGetUserSecurityLevelByUserId(USR.[HRUserId]) AS 'SecurityLevel'
		, @SessionId AS [SessionId]
		, USR.[IsActive]
		, USR.[IsDeleted]
	FROM
		[WISE_AuthenticationControl].[dbo].[vwAC_UsersAppAuthentication] AS USR WITH (NOLOCK)
	...
	BEGIN
		...
		UPDATE [WISE_AuthenticationControl].[dbo].AC_Sessions SET
			...
			, Groups = @Groups
		FROM
			[WISE_AuthenticationControl].[dbo].AC_Sessions AS SESS WITH (NOLOCK)
...
/**
UPDATE [WISE_AuthenticationControl].[dbo].AC_Sessions SET LastAccessedOn = GetDate(), SessionTerminated = 0 WHERE SessionID = 212240;
EXEC dbo.custAC_UsersCrmAuthentication 'DevUser', 'NexSense', 432943, 'SSE_CMS_CORS';
	...
	/** Check session validation. */
	SELECT * FROM [WISE_AuthenticationControl].[dbo].AC_Sessions AS SESS WITH (NOLOCK) WHERE (SESS.SessionID = @SessionId) AND (SESS.SessionTerminated = 0) AND (SESS.LastAccessedOn BETWEEN @Start AND @End);
	PRINT 'Session is bad. START: ''' + CAST(@Start AS VARCHAR) + '''| END: ''' + CAST(@End AS VARCHAR) + '''';
...
SELECT * FROM [WISE_AuthenticationControl].[dbo].AC_Users AS HRU WITH (NOLOCK) WHERE UserID = 10001; */