...
GO
/** Declarations */
DECLARE @AccountID BIGINT = 191148;
SELECT TOP 2 * FROM dbo.MS_AccountSubmits ORDER BY AccountSubmitID DESC;
SELECT * FROM MS_Accounts WHERE AccountID = @AccountID;
SELECT TOP 100 * FROM dbo.MS_AccountSubmitMs 
WHERE (AccountSubmitID IN (SELECT TOP 2 AccountSubmitID FROM dbo.MS_AccountSubmits WHERE AccountId = @AccountID ORDER BY AccountSubmitID DESC))
