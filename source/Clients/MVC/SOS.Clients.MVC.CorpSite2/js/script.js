...
$.include('/js/AppService.js');
$(function ()
{
	if ($('#coin-slider').length) $.include('/js/coin-slider.js');
	if ($('.tabs').length) $.include('/js/tabs.js');
	if ($('.fixedtip').length || $('.clicktip').length || $('.normaltip').length) $.include('/js/jquery.atooltip.pack.js');
	if ($('#contact-form').length || $('#newsletter-alt-form').length || $('#newsletter-form').length || $('#enroll-form').length || $('#lead-form').length || $('#careers-form').length) $.include('/js/forms.js');
	...
	$('.list-1 li')
		...
		.wrap('<div></div>')
		.hover(
			function () { $(this).parent().prev().animate({ height: 0, marginTop: 10, backgroundColor: '#858585' }, 100, function () { $(this).animate({ height: 19, marginTop: 0, backgroundColor: '#04385C' }, 100); }); }
			, function() { $(this).parent().prev().stop().animate({ height: 0, marginTop: 10, backgroundColor: '#858585' }, 100, function() { $(this).animate({ height: 19, marginTop: 0, backgroundColor: '#fff' }, 100); }); });
	$('header .links li a').append('<span></span>').hover(
		...
		, function ()
		{
			$(this).stop().animate({ backgroundPosition: '0 -156px', color: '#CF0C0D' }, { duration: 200, queue: false }).find('>span').animate({ backgroundColor: '#8d6f1d', marginLeft: 0, width: 0 }, { duration: 100, queue: false, complete: function () { $(this).animate({ backgroundColor: '#CF0C0D', marginLeft: -12, width: 23 }, 100); } });
		});
...
$(function () {
	/** Entry point for AppService. */
	SOS.AppService.InitDownloadBrochure();
	SOS.AppService.InitRepAndDealerBrochure();
	...
	switch (sectionName) {
		...
		case 'Services':
			...
			break;
		case 'Plans':
			$('#rootPlans').addClass('current');
			break;
		case 'Technology':
			$('#rootTechnology').addClass('current');
			break;
		case 'WhyUs':
			$('#rootWhyUs').addClass('current');
			break;
		case 'Dealers':
			$('#rootDealers').addClass('current');
			break;
		case 'FAQs':
			$('#rootFAQs').addClass('current');
			break;
		case 'ContactUs':
